...
olc2C02::olc2C02()
{
    	// He got the following palette table from the nesdev wiki
	// 2C02 Palettes under the PPU palettes page
...
{
    uint8_t data = 0x00; // Temp Variable

    // Creating a switch statement, which he will explain in another video, I think.
    // But for now they will not do anything either.
    switch (addr)
    {
        case 0x0000: // Control
            break;
...
{
    // Placeholder information for now, as stated in the video.
    uint8_t data = 0x00;
    addr &= 0x3FFF;
...
{
    addr &= 0x3FFF; // Masking the address, just in case the PPU tries to write outside of range of its' bus.
...
void olc2C02::clock()
{
    ...
    //Advance renderer - it never stops, it's relentless... So he says. Challenge accepted. /s
    cycle++;
    if ( cycle >= 341 )
    {
        cycle = 0;
